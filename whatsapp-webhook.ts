 ...
 * 
 * It automatically detects the source and processes accordingly.
 * 
...
serve(async (req) => {
  ...
  try {
    ...
    let hospitalName: string = 'the hospital'; // Default fallback
    
    // First try to find hospital by whatsapp_number (with or without + prefix)
    ...
    if (hospital) {
      ...
      // Fallback: get the first hospital with whatsapp enabled or any hospital
